...
#read_file <- openxlsx::read.xlsx(xlsxFile = "C:\\Users\\umadu\\Documents\\ecotoxicology-thesis\\Data\\LC_EC50_new.xlsx", 1)

#column_names <- names(read_file)
#column_names

#EXTRACTING columns which will be used
#use_df <- read_file %>% select(contains(c("CAS", "Smiles")))


#Creating function for shiny app

tsne_Data <- function(data_df, fp_type){
  use_df <- data_df
  #names(use_df)[length(use_df)] <- "SMILES"
  duplicates <- duplicated(use_df)
  use_df <- use_df[!duplicates,]
  ...
  fps_final <- lapply(not_null, get.fingerprint, type = fp_type, fp.mode = "bit")
  ...
  # Complete data
  full_data <- inner_join(use_df, joined, by = c("CAS", "Smiles"), copy = FALSE)
  
  
  #### To get index of V1 column
  col_index <- which(colnames(full_data) == "V1")
  ...
  full_data[,col_index:length(full_data)] <- lapply(full_data[,col_index:length(full_data)], as.numeric)
  ...
  full_data[, col_index: length(full_data)] <- normalise_mm(full_data[, col_index: length(full_data)])
  return(full_data)
}

# fp_type <- "graph"
# try1 <- tsne_Data(use_df, fp_type)


create_tsne <- function(data_df, fp_type){
  set.seed(42)
  full_data <- data_df
  tsne.res <- Rtsne(full_data, perplexity = 10, check_duplicates = FALSE, pca = FALSE, verbose = TRUE)
  tsne_out_DF <- data.frame(x = tsne.res$Y[,1], y = tsne.res$Y[,2])
  g <- ggplot(tsne_out_DF,aes(x,y)) +  geom_point(alpha = 0.8) + theme_bw() + ggtitle(paste("tSNE - ", fp_type, sep = " TEST"))
  
  #combined tsne_out_DF with original cas and smiles
  all_data <- cbind(full_data[,1:2], tsne_out_DF)
  
  ret <- list(g, all_data)
  return(ret)
  #ggplotly(g)
}
# plott <- create_tsne(try1)
# plott
###############
##plott <- create_tsne(try1)
##ggplotly(plott)
...
#TSNE
# set.seed(42)
# tsne.res <- Rtsne(full_data, perplexity = 30, check_duplicates = FALSE, pca = FALSE, verbose = TRUE)
# tsne_out_DF <- data.frame(x = tsne.res$Y[,1], y = tsne.res$Y[,2])
...
#Generating PCA
# res <- prcomp(full_data[,col_index:length(full_data)])
